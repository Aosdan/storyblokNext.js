...
const RollingGallery = ({ autoplay = false, pauseOnHover = false, images = [] }) => {
  ...
  useEffect(() => {
    const getIsSmall = () => (typeof window !== 'undefined' ? window.innerWidth <= 640 : false);
    setIsScreenSizeSm(getIsSmall());
    const handleResize = () => setIsScreenSizeSm(getIsSmall());
      ...
      <div className="flex h-full items-center justify-center [perspective:1000px] [transform-style:preserve-3d]">
        ...
        >
          {images.map((url, i) => (
            <div
              key={i}
              className="group absolute flex h-fit items-center justify-center p-[10%] [backface-visibility:hidden] md:p-[9%]"
              style={{
                width: `${faceWidth}px`,
